...
if len(faces) == 0:
    ...
    for (x, y, w, h) in faces:
        ...
        roi_gray = gray[y:y + h, x:x + w]  # Converting the captured face to gray scale
        roi_color = img[y:y + h, x:x + w]  # Converting the captured back to color
        eyes = eyeClassifier.detectMultiScale(roi_gray, 1.2, 4)  # Detect eyes in the captured face
    ...
    # Outputting the file with the name: imageFileName(FirstClassifierName&SecondClassifierName).png
    faceClassifierOutput = faceClassifierFile.split(".")  # -> haarcascade_frontalface_default
    eyeClassifierOutput = eyeClassifierFile.split(".")  # -> haarcascade_eye
